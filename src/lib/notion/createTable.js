...
import { BLOG_INDEX_ID, NOTION_TOKEN, API_ENDPOINT } from "@/lib/notion/server-constants";
...
async function getExistingexistingBlockId() {
  ...
  if (!res.ok) {
    throw new Error(`failed to get existing block id, request status: ${res.status}`);
  }
  const data = await res.json();
  const id = Object.keys(data ? data.recordMap.block : {}).find((id) => id !== BLOG_INDEX_ID);
  return id || uuid();
}
